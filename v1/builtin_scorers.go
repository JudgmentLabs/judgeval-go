...
type InstructionAdherenceScorerParams struct {
	Threshold  *float64
	Name       *string
	StrictMode *bool
	Model      *string
...
type InstructionAdherenceScorer struct {
	*apiScorer
	requiredParams []string
}

func (f *BuiltInScorersFactory) InstructionAdherence(params InstructionAdherenceScorerParams) *InstructionAdherenceScorer {
	scorer := newAPIScorer(
		APIScorerTypeInstructionAdherence,
		getFloat(params.Threshold, 0.5),
		getString(params.Name, ""),
		getBool(params.StrictMode, false),
		getString(params.Model, ""),
	...
	return &InstructionAdherenceScorer{
		apiScorer:      scorer,
		requiredParams: []string{"input", "actual_output"},
	}
}

func (s *InstructionAdherenceScorer) GetScorerConfig() *models.ScorerConfig {
	return s.apiScorer.toScorerConfig(s.requiredParams)
}

type DerailmentScorerParams struct {
	Threshold  *float64
	Name       *string
	StrictMode *bool
	Model      *string
...
type DerailmentScorer struct {
	*apiScorer
	requiredParams []string
}

func (f *BuiltInScorersFactory) Derailment(params DerailmentScorerParams) *DerailmentScorer {
	scorer := newAPIScorer(
		APIScorerTypeDerailment,
		getFloat(params.Threshold, 0.5),
		getString(params.Name, ""),
		getBool(params.StrictMode, false),
		getString(params.Model, ""),
	...
	return &DerailmentScorer{
		apiScorer:      scorer,
		requiredParams: []string{"input", "actual_output"},
	}
}

func (s *DerailmentScorer) GetScorerConfig() *models.ScorerConfig {
	return s.apiScorer.toScorerConfig(s.requiredParams)
}
